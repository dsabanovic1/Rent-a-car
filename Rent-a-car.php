			 ...
			 <article class="bottomContent">
				...
				<content>
				      ...
				      if(isset($_REQUEST['ime']) && isset($_REQUEST['email']))
				   {
						...
						$xml->load('XMLRegistracija.xml');
						$imeprezime=htmlspecialchars($_REQUEST['ime']);
                         $email=htmlspecialchars($_REQUEST['email']);
						 $nadimak=htmlspecialchars($_REQUEST['nadimak']);
						 $sifra=htmlspecialchars($_REQUEST['sifra']);
						$rootTag = $xml->getElementsByTagName("document")->item(0);
						
						$dataTag = $xml->createElement("data");
						$imePrezimeTag=$xml->createElement("ImePrezime", $imeprezime);
						$EmailTag=$xml->createElement("Email", $email);
						$dataTag->appendChild($imePrezimeTag);
						$dataTag->appendChild($EmailTag);
						$nadimakTag=$xml->createElement("Username", $nadimak);
						$PasswordTag=$xml->createElement("Password", $sifra);
						$dataTag->appendChild($nadimakTag);
						$dataTag->appendChild($PasswordTag);

						$rootTag->appendChild($dataTag);
						$xml->save("XMLRegistracija.xml");
				   }
				?>
